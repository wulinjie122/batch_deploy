...
wwwHosts = [
    ...
    # 搜索服务器
    {'ip': '10.0.204.195', 'testUser': testUser, 'rootUser': rootUser1,
     'path': ['searchEngine', 'searchEngine2', 'searchEngine3']},
    {'ip': '10.0.204.196', 'testUser': testUser, 'rootUser': rootUser1,
     'path': ['searchEngine1', 'searchEngine2', 'searchEngine3']}
]

# UAT环境主机列表
uatHosts = [
    {'ip': '10.0.204.204', 'testUser': testUser, 'rootUser': rootUser2,
     'path': ['read1', 'read2', 'write', 'searchEngine']},
...
def batchDeploy(cType):
    ...
    for host in remoteHosts:
        ...
        for path in host['path']:
            print
            'deploying' + path
            child = pexpect.spawn('rsync -rltDvu /home/aiuap_jc/deploy/deploySrc/eshop.war/ test@' + host[
                'ip'] + ':/jboss/eshop/jboss_' + path + '/server/web/deploy/eshop.war')
            i = child.expect([pexpect.TIMEOUT, ssh_newkey, 'password: '])
            if (i == 0):
                logging.error('SSH could not login. Here is what SSH said:')
                logging.error(child.before, child.after)
...
def batchCmd(ctype, cmd):
    # ctype = int(input('please select the command type: 1-uat;2-www \(input 1 or 2\)'))
    ...
    if (ctype == 1):
        ...
        msg = 'www'
    if (cmd == ''):
        cmd = int(input('please select the command: 1-shutdown all hosts 2-reboot all hosts \(input 1 or 2\)'))
    # 要执行的命令
    ...
    logging.info(
        '---------------------now \"' + msg + '\" execute \"' + cmdStr + '\" is over !------------------------\n')
...
def main():
    # 选择执行命令的主机
    hostType = int(input('please select the host type: 1-----uat;2-----www;\n\(input 1or2\)'))
    # 选择命令的类型
    # 1：批量执行ssh命令
    # 2：批量rsync发布
    # 3：一键执行部署命令（带备份）
    ...
    # 5：版本回滚
    # 6：待定
    dtype = int(input('please select the command type:\n\
                          1-batch execute ssh;\n\
